...
from DynamicConnect4Interface import draw_table_score, write_2_file


# Basic Minimax or Alpha Beta Pruning with depth-first search
class game_v0():
    def __init__(self):
        self.ROWS = 7
        self.COLS = 7
        ...
        self.board_A = [[self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym],
                        [self.MinSym, self.EmpSym, self.MaxSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.EmpSym, self.MinSym, self.EmpSym, self.EmpSym, self.MaxSym],
                        [self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.MinSym, self.MaxSym, self.MaxSym],
                        [self.EmpSym, self.EmpSym, self.MinSym, self.EmpSym, self.EmpSym, self.MinSym, self.MaxSym],
                        [self.EmpSym, self.EmpSym, self.EmpSym, self.MaxSym, self.MinSym, self.EmpSym, self.EmpSym]]
        self.board_B = [[self.MinSym, self.MinSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym],
                        [self.MaxSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.MaxSym, self.EmpSym, self.EmpSym, self.MinSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.MaxSym, self.MinSym, self.EmpSym, self.EmpSym, self.MaxSym],
                        [self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.MinSym, self.MaxSym, self.EmpSym],
                        [self.EmpSym, self.MaxSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym],
                        [self.MinSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym]]
        self.board_C = [[self.EmpSym, self.EmpSym, self.MinSym, self.EmpSym, self.MaxSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.MinSym, self.EmpSym, self.MaxSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.MaxSym, self.EmpSym, self.MinSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.MaxSym, self.EmpSym, self.MinSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.MinSym, self.EmpSym, self.MaxSym, self.EmpSym, self.EmpSym],
                        [self.EmpSym, self.EmpSym, self.MaxSym, self.EmpSym, self.MinSym, self.EmpSym, self.EmpSym]]
    ...
    def move(self, board, player):
        ...
        if self.USE_RAND_MOVE:
            return self.rand_move(moves)
        elif self.USE_MOVE_ORDERING:
            return self.score_sort(board, moves, player)
        else:
            return moves
    # Randomize move generation
    def rand_move(self, moves):
        random.shuffle(moves)
        return moves

    # Move sorting by scores
    def score_sort(self, node, moves, player):
        scores = []
        for move in moves:
            scores.append(self.is_winning(self.do_move(node, move, player)))
            self.undo_move(node, move, player)
        if player:
            newmoves = [move for _, move in sorted(zip(scores, moves), reverse=True)]
        else:
            newmoves = [move for _, move in sorted(zip(scores, moves), reverse=False)]
        return newmoves
    ...
    def find_best_move(self, board, player):
        ...
        for move in moves:
            board = self.do_move(board, move, player)
            if self.USE_PRUNING:
                val_of_move = self.alphabeta(board, 1, not player, self.MIN, self.MAX)
            else:
                val_of_move = self.minimax(board, 1, not player)
            board = self.undo_move(board, move, player)
...
if __name__ == "__main__":
    ...
    #     write_2_file(board, game.no_of_nodes, game.depth_limit, "Board_A")
    #     game.no_of_nodes = 0
    #     game.no_of_plies = []
    #     player = not player
    ...
    while game.depth_limit < 7:
        with open("Board_C.txt", "a") as myfile:
            myfile.write("Depth = " + str(game.depth_limit) + "\n")
            myfile.write("Without ordering\n")
        game.USE_MOVE_ORDERING = False
        game.no_of_nodes = 0
        best_move = game.find_best_move(board, player)
        write_2_file(board, game.no_of_nodes, game.depth_limit, "Board_C")

        with open("Board_C.txt", "a") as myfile:
            myfile.write("\nWith ordering\n")
        game.USE_MOVE_ORDERING = True
        game.no_of_nodes = 0
        best_move = game.find_best_move(board, player)
        write_2_file(board, game.no_of_nodes, game.depth_limit, "Board_C")
